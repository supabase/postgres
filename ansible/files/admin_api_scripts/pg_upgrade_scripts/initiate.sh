...
# handle_upgrade_prerequisites applies changes to the database to prevent known
...
function initiate_upgrade {
	echo "1. Handling upgrade prerequisites"
	handle_upgrade_prerequisites
	...
	echo "2. Extracting pg_upgrade binaries"
		...
		if [ "$IS_NIX_BASED_SYSTEM" = "false" ]; then
			if [ ! -f /nix/var/nix/profiles/default/etc/profile.d/nix-daemon.sh ]; then
				if ! command -v nix >/dev/null; then
					echo "2.1. Nix is not installed; installing."

					if [ -f "$NIX_INSTALLER_PACKAGE_PATH" ]; then
						echo "2.1.1. Installing Nix using the provided installer"
						...
						echo "2.1.1. Installing Nix using the official installer"
...
EXTRA_NIX_CONF
					...
					echo "2.1. Nix is installed; moving on."
				fi
			fi
		fi

		echo "2.2. Fetching store path for flake revision: $NIX_FLAKE_VERSION"
		# shellcheck disable=SC1091
	...
	echo "3. Fixing potential errors generated by dpkg"
	echo "3.1 Killing off any old hanging apt-get processes"
	...
	echo "4. Installing libprotobuf-c1 and libicu66 if missing"
	...
	echo "5. Setup locale if required"
	...
	if [ -z "$IS_CI" ] && [ -z "$IS_LOCAL_UPGRADE" ]; then
		...
		echo "6. Determining block device to mount"
		# lsb release
	...
	echo "8. Disabling extensions and generating post-upgrade script"
	handle_extensions

	echo "9.1. Granting SUPERUSER to postgres user"
	run_sql -c "ALTER USER postgres WITH SUPERUSER;"

	if [ "$OLD_BOOTSTRAP_USER" = "postgres" ]; then
		echo "9.2. Swap postgres & supabase_admin roles as we're upgrading a project with postgres as bootstrap user"
	...
	echo "10. Creating new data directory, initializing database"
...
EOF
	...
	echo "11. Stopping postgres; running pg_upgrade"
	...
	echo "12. Copying custom configurations"
	...
	echo "13. Copying sql files generated by pg_upgrade"
