    ...
    ''
      ...
      with subtest("Switch to OrioleDB and show rewind config"):
        ...
        installed_extensions = server.succeed(
          "psql -U postgres -d postgres -t -A -c \"SELECT extname FROM pg_extension WHERE extname = 'orioledb';\""
        ).strip()
        ...
        rewind_settings = server.succeed(
          "psql -U postgres -d postgres -t -A -c \"SELECT name || ' = ' || setting FROM pg_settings WHERE name LIKE 'orioledb.%rewind%' OR name LIKE 'orioledb.%main_buffers%' ORDER BY name;\""
        ).strip()
        print(f"OrioleDB rewind settings:\n{rewind_settings}")

        # Assert rewind is enabled
        rewind_enabled = server.succeed(
          "psql -U postgres -d postgres -t -A -c \"SHOW orioledb.enable_rewind;\""
        ).strip()
        ...
        queue_len = server.succeed(
          "psql -U postgres -d postgres -t -A -c \"SELECT orioledb_get_rewind_queue_length();\""
        ).strip()
        evicted_len = server.succeed(
          "psql -U postgres -d postgres -t -A -c \"SELECT orioledb_get_rewind_evicted_length();\""
        ).strip()
      ...
      with subtest("Basic rewind"):
        ...
        server.succeed(
          "psql -U postgres -d postgres -c \"DROP TABLE IF EXISTS rewind_test; CREATE TABLE rewind_test(x serial) USING orioledb; INSERT INTO rewind_test SELECT FROM generate_series(1, 100);\""
        )

        # Capture xid, oxid, and hash in a single query — matches bash script
        ids = server.succeed(
          "psql -U postgres -d postgres -t -A -c \"SELECT pg_current_xact_id()::text::int4, orioledb_get_current_oxid()::int8, md5(string_agg(x::text, '.' ORDER BY x)) FROM rewind_test;\""
        ).strip()
        parts = ids.split("|")
        xid = parts[0]
        oxid = parts[1]
        ...
        server.succeed(
          "psql -U postgres -d postgres -c \"INSERT INTO rewind_test SELECT FROM generate_series(1, 10); INSERT INTO rewind_test SELECT FROM generate_series(1, 10); INSERT INTO rewind_test SELECT FROM generate_series(1, 10); INSERT INTO rewind_test SELECT FROM generate_series(1, 10);\""
        )
        dirty_count = server.succeed(
          "psql -U postgres -d postgres -t -A -c \"SELECT count(*) FROM rewind_test;\""
        ).strip()
        ...
        server.succeed(
          f"psql -U postgres -d postgres -c \"SELECT orioledb_rewind_to_transaction({xid}, {oxid});\" 2>&1 || true"
        )
        ...
        time.sleep(5)
        server.succeed(
          "until psql -U postgres -d postgres -t -A -c 'SELECT 1' 2>/dev/null; do sleep 1; done"
        )
        print("Server is back up")

        # Verify
        post_hash = server.succeed(
          "psql -U postgres -d postgres -t -A -c \"SELECT md5(string_agg(x::text, '.' ORDER BY x)) FROM rewind_test;\""
        ).strip()
      ...
      with subtest("Rewind under buffer pressure"):
        ...
        server.succeed(
          "psql -U postgres -d postgres -c \"DROP TABLE IF EXISTS pressure_test; CREATE TABLE pressure_test(x serial) USING orioledb;\""
        )

        # Insert data in 50 batches of 100 rows each (5000 rows across 50 transactions)
        for batch in range(50):
          server.succeed(
            "psql -U postgres -d postgres -c \"INSERT INTO pressure_test SELECT FROM generate_series(1, 100);\""
          )

        # Capture checkpoint state in a single query
        cp_ids = server.succeed(
          "psql -U postgres -d postgres -t -A -c \"SELECT pg_current_xact_id()::text::int4, orioledb_get_current_oxid()::int8, count(*) FROM pressure_test;\""
        ).strip()
        ...
        after_count = server.succeed(
          "psql -U postgres -d postgres -t -A -c \"SELECT count(*) FROM pressure_test;\""
        ).strip()
        ...
        server.succeed(
          f"psql -U postgres -d postgres -c \"SELECT orioledb_rewind_to_transaction({cp_xid}, {cp_oxid});\" 2>&1 || true"
        )
