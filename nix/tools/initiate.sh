...
get_bindir_from_flake() {
    local flake_url="$1"
    nix build "$flake_url" --no-link --print-out-paths
}

# Check if nix is available
if ! command -v nix &> /dev/null; then
    echo "ERROR: nix command not found. Please ensure Nix is installed and in your PATH."
    exit 1
fi

# First argument is the flake URL for the old version
if [[ $1 == *#* ]]; then
    OLDVER=$(get_bindir_from_flake "$1")
    if [ ! -d "$OLDVER/bin" ]; then
        echo "ERROR: $1 does not look like a valid Postgres install"
        exit 1
    fi
else
    echo "Please provide a valid flake URL for the old Postgres version"
    exit 1
fi

# Second argument is the flake URL for the new version
if [[ $2 == *#* ]]; then
    NEWVER=$(get_bindir_from_flake "$2")
    if [ ! -d "$NEWVER/bin" ]; then
        echo "ERROR: $2 does not look like a valid Postgres install"
        exit 1
    fi
else
    echo "Please provide a valid flake URL for the new Postgres version"
    exit 1
fi
...
function initiate_upgrade {
    ...
    # /end not need as libs, binaries are handled already by nix
