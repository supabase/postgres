...
COMMENT ON EXTENSION "supabase-dbdev" IS 'Install packages from the database.dev registry';
...
CREATE FUNCTION pgtle."supabase-dbdev.control"() RETURNS text
    LANGUAGE sql
    AS $_X$SELECT $_pgtle_i_$default_version = '0.0.2'
comment = 'Install packages from the database.dev registry'
relocatable = false
superuser = false
trusted = false
requires = 'pg_tle'
